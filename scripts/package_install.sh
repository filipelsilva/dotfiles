...
if ( command -v apt-get &> /dev/null ); then
    ...
    sudo apt-get install -qq -y zsh git curl wget glances htop neovim\
        ...
        tmux tmate make lynx ripgrep fd-find silversearcher-ag nodejs npm\
        neofetch screenfetch entr rlwrap exuberant-ctags hexyl bat cmake\
        shellcheck lazygit golang

elif ( command -v pacman &> /dev/null ); then

    sudo pacman -Syu --noconfirm
    sudo pacman -S --noconfirm zsh git curl wget glances htop python-pip\
        python python-setuptools gcc valgrind gdb unzip zip tmux tmate\
